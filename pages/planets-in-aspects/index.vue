...
export default {
  ...
  fetch({ store }) {
    ...
    if (_.isEmpty(store.state.dropdowns.aspects)) {
      promises.push(getDropdownsData("aspects", "setAspects", store));
    }
    return Promise.all(promises);
  },
  computed: mapState({
    planets: state => state.dropdowns.planets,
    aspects: state => state.dropdowns.aspects
  })
};
</script>
